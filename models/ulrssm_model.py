...
class ULRSSM_Model(BaseModel):
    def __init__(self, opt):
        ...
        if self.with_refine > 0:
            opt['is_train'] = True
        super(ULRSSM_Model, self).__init__(opt)
    ...
    def feed_data(self, data):
        ...
        # # Test different k values from 20 to 200
        # for k in range(20, 301, 5):
        #     times = []
        #     # Run 10 times for each k
        #     for _ in range(6):
        #         # Take k dimensions of spectral data
        #         evals_x_k = evals_x[:,:k]
        #         evals_y_k = evals_y[:,:k] 
        #         evecs_trans_x_k = evecs_trans_x[:,:k,:]
        #         evecs_trans_y_k = evecs_trans_y[:,:k,:]

        #         start_time = time.time()
        #         Cxy, Cyx = self.networks['fmap_net'](feat_x, feat_y, evals_x_k, evals_y_k, evecs_trans_x_k, evecs_trans_y_k)
        #         end_time = time.time()
        #         times.append((end_time - start_time) * 1000)  # Convert to ms
            
        #     avg_time = np.mean(times[1:])
        #     # print(f"k={k}: {avg_time:.2f}ms")
    ...
    def validation(self, dataloader, tb_logger, update=True):
        ...
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = False
        super(ULRSSM_Model, self).validation(dataloader, tb_logger, update)
        if 'permutation' in self.networks:
            self.networks['permutation'].hard = False
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = True
