...
def init_loggers(opt):
    tb_logger = init_web_logger(opt['path']['experiments_root'])
    return tb_logger
...
def train_pipeline(root_path):
    ...
    # TODO: ULRSSM's config does not allow easy overriding, just use wandb for now
    if True or opt['wandb']:
        import wandb
        name = opt['name']
        model_string = opt['type'].lower()[:-6]
